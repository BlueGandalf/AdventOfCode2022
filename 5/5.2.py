...
with open(os.path.join(os.path.dirname(__file__), '5.txt')) as f:
    ...
    for line in f:
        if ('[' in line): # crates in initial configuration
            ...
            movingCrates = crateConfiguration[int(fromPile)][(0-int(quantity)):];
            del crateConfiguration[int(fromPile)][(0-int(quantity)):]
            crateConfiguration[int(toPile)] += movingCrates;
